...
class DiscTrack:
    def __init__(self, name: str) -> None:
        self.name = name
        self.directory = os.path.join(DATA_DIRECTORY, name)
        self.read_camera_settings()
        self.read_tracks()

    def deproject(self) -> tuple:
        x = 0.5 * self.camera_settings["d"] * (self.xl + self.xr) / (self.xr - self.xl)
        z = -(0.5 * self.camera_settings["d"] * (self.zl + self.zr) / (self.xr - self.xl)) + self.camera_settings["h"]
        y = self.camera_settings["d"] * self.camera_settings["f"] / (self.xr - self.xl) - self.camera_settings["c"]
        return (x, y, z)
    ...
    def read_camera_settings(self) -> None:
        with open(os.path.join(DATA_DIRECTORY, self.name, "camera_settings.yaml")) as file:
            self.camera_settings = yaml.safe_load(file)
        self.camera_settings["f"] = self.camera_settings["focal_length"] * self.camera_settings["resolution"][0] / self.camera_settings["sensor_width"]
