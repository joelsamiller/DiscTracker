...
class DiscTrack:
    ...
    def deproject(self) -> tuple:
        ...
        z = -(0.5 * self.camera_settings["d"] * (self.zl + self.zr) / (self.xl - self.xr)) + self.camera_settings["h"]
        y = self.camera_settings["d"] * self.camera_settings["f"] / (self.xl - self.xr) - self.camera_settings["c"]
        return (x, y, z)
